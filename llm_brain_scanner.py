"""
SEMANTIC PRIMITIVES - LLM BRAIN SCANNER (GPU)
==============================================
Scans decoder LLMs layer by layer on GPU.
Uses last-token pooling (correct for autoregressive models).

Usage:
    py llm_brain_scanner.py Qwen/Qwen2.5-1.5B
    py llm_brain_scanner.py Qwen/Qwen2.5-7B
    py llm_brain_scanner.py Qwen/Qwen2.5-14B
"""
...
if len(sys.argv) < 2:
    print("Usage: py llm_brain_scanner.py <model-name>")
    print("Example: py llm_brain_scanner.py Qwen/Qwen2.5-7B")
    sys.exit(1)

MODEL_NAME = sys.argv[1]
# Create safe filename tag: "Qwen/Qwen2.5-7B" -> "Qwen2.5-7B"
...
def get_all_layer_embeddings(text, model, tokenizer):
    """
    Get sentence embedding at EVERY layer of a decoder LLM.
    Uses LAST TOKEN position (correct for autoregressive models).
    """
    ...
    for layer_hidden in hidden_states:
        last_token = layer_hidden[:, -1, :].float()
        layer_embeddings.append(last_token.squeeze(0).cpu().numpy())
...
def run_experiment():
    ...
    show_every = 1 if total_layers <= 15 else 2
    show_indices = list(range(0, total_layers, show_every))
    if (total_layers - 1) not in show_indices:
        show_indices.append(total_layers - 1)
    
    print(f"\n  {'Primitive':<16}", end="")
    for idx in show_indices:
        print(f" {layer_labels[idx]:>5}", end="")
    print("   Peak")
    print("  " + "-" * (16 + 6 * len(show_indices) + 8))
    ...
    for p_idx, prim_name in enumerate(prim_names):
        ...
        for idx in show_indices:
            print(f" {row[idx]:.2f}", end="")
        print(f"   {layer_labels[peak_layer]} ({peak_val:.2f})")
    
    print(f"\n  {'ORTHOGONALITY':<16}", end="")
    for idx in show_indices:
        print(f" {orthogonality_by_layer[idx]:.2f}", end="")
    ...
    fig, axes = plt.subplots(2, 1, figsize=(max(16, total_layers * 0.6), 10), 
    ...
    # Always show numbers — adjust font size based on layer count
    font_size = 7 if total_layers <= 30 else 5
    ...
    print(f"\n   Model: {MODEL_NAME}")
    print(f"   Layers scanned: {total_layers} (embedding + {n_layers} transformer layers)")
    ...
    # Anatomy breakdown
    third = n_layers // 3
    enc_layers = list(range(0, third + 1))
    mid_layers = list(range(third + 1, 2 * third + 1))
    dec_layers = list(range(2 * third + 1, total_layers))
    
    enc_avg = np.mean([avg_consistency_by_layer[i] for i in enc_layers])
    mid_avg = np.mean([avg_consistency_by_layer[i] for i in mid_layers])
    dec_avg = np.mean([avg_consistency_by_layer[i] for i in dec_layers])
    
    enc_ort = np.mean([orthogonality_by_layer[i] for i in enc_layers])
    mid_ort = np.mean([orthogonality_by_layer[i] for i in mid_layers])
    dec_ort = np.mean([orthogonality_by_layer[i] for i in dec_layers])
    
    print(f"\n   Anatomy breakdown (encoding / reasoning / decoding):")
    print(f"   Encoding  (Emb-L{third}):  consistency={enc_avg:.4f}  orthogonality={enc_ort:.4f}")
    print(f"   Reasoning (L{third+1}-L{2*third}): consistency={mid_avg:.4f}  orthogonality={mid_ort:.4f}")
    print(f"   Decoding  (L{2*third+1}-L{n_layers}): consistency={dec_avg:.4f}  orthogonality={dec_ort:.4f}")
    ...
    for p_idx, prim_name in enumerate(prim_names):
        ...
        phase = "ENCODING" if peak in enc_layers else "REASONING" if peak in mid_layers else "DECODING"
        print(f"   {prim_name:<16} peaks at {layer_labels[peak]:>4} ({val:.4f}) [{phase}]")
